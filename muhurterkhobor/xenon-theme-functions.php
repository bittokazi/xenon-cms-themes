...
xenon_custom_link('archive','xenon-themes/muhurterkhobor/archive.php');
xenon_custom_link('getarchive','xenon-themes/muhurterkhobor/getarchive.php');
...
if (isset($_GET['action']) && isset($_GET['registertheme']) && $_GET['action']=="true" && $_GET['registertheme']=="muhurterkhobor") {
...
if (isset($_GET['action']) && isset($_GET['registertheme']) && $_GET['action']=="true" && $_GET['registertheme']=="muhurterkhobor") {
$result4 = @mysql_query("SELECT * FROM frndzk_menu WHERE menuloc='upper-menu_menu249ony_bdwave'");
if ( @mysql_num_rows($result4) > 0 ) {

while($general4 = @mysql_fetch_array($result4)) {
$query="UPDATE frndzk_menu SET menuloc = 'upper-menu_menu249ony_muhurterkhobor'
WHERE id = '$general4[id]'";
mysql_query($query);
}
...
if ( @mysql_num_rows($result1) > 0 ) {
while($general1 = @mysql_fetch_array($result1)) {
echo'   <a href="'.home_url().xenon_link_post().$general1['shortname'].'"><div class="lof-main-item">
        		<img src="'.get_image_bdwave($general1['postcontent']).'" title="Newsflash 2" height="200" width="300"></a>           
                 <div class="lof-main-item-desc">
                <h3><a target="_parent" title="Newsflash 2" href="'.home_url().xenon_link_post().$general1['shortname'].'">'.$general1['post'].'</a></h3>
...
echo'<!--tab-->
    <div style="float:left; margin-left: 2px; margin-top: 5px; width: 250px; font: 0.85em solaimanlipi;">
		...
		$it=0;
          ...
          echo'<div id="view'.$general['id'].$its.'" class="tabcontent"><ul>';
if ( $it == 0 ) {
$result1 = @mysql_query("SELECT * FROM frndzk_post ORDER by id desc LIMIT 0,70");
if ( @mysql_num_rows($result1) > 0 ) {
while($general1 = @mysql_fetch_array($result1)) {
echo'<li><a target="_parent" title="Newsflash 2" href="'.home_url().'/'.xenon_link_post().$general1['shortname'].'">'.$general1['post'].'</a></li>';
}
...
if ( $it == 5 ) {
$result1c = @mysql_query("SELECT * FROM bdwave_cc ORDER by count desc LIMIT 0,30");
if ( @mysql_num_rows($result1c) > 0 ) {
while($general1c = @mysql_fetch_array($result1c)) {
...
if ( $it == 1 ) {
$result1c = @mysql_query("SELECT * FROM bdwave_counter ORDER by count desc LIMIT 0,30");
if ( @mysql_num_rows($result1c) > 0 ) {
while($general1c = @mysql_fetch_array($result1c)) {
$postidc=$general1c['post'];
...
if ( @mysql_num_rows($resultslider) > 0 ) {
while($generalsli = @mysql_fetch_array($resultslider)) {
echo"<a href=\"".home_url()."/".xenon_link_post().$generalsli['shortname']."\"><img src=\"".get_image_bdwave($generalsli['postcontent'])."\" title=\"Description from Image Title\" /></a>
...
xmlhttp.open("POST","http://'.$_SERVER['HTTP_HOST'].'/xenon-themes/muhurterkhobor/upload.php",true);
...
$file = fopen("../xenon-themes/muhurterkhobor/content/bn.php","w");
